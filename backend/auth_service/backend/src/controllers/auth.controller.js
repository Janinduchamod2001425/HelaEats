  ...
  try {
    ...
    if (isAdminRequest) {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ message: "Invalid role for admin signup" });
      }
    } else {
    ...
    // ✉️ Send credentials email if it's an admin-created user
    if (isAdminRequest) {
      await sendCredentialsEmail({
        ...
        role: finalRole,
        email,
        password, // NOTE: This is the original password from req.body
      });
    }
...
export const getUserById = async (req, res) => {
  try {
    const user = await Auth.findById(req.params.userId).select(
      "name email role"
    );
