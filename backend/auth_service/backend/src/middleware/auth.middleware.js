...
export const protectCustomerRoutes = async (req, res, next) => {
  try {
    ...
    if (!req.user || req.user.role !== "customer") {
      return res
        .status(403)
        .json({ error: "Unauthorized - Only customers can access this route" });
    }
  ...
  try {
    ...
    if (!req.user || req.user.role !== "restaurant_admin") {
      return res.status(403).json({
        error: "Unauthorized - Only restaurant admins can access this route",
      });
    }
...
export const protectDeliveryPersonnelRoutes = async (req, res, next) => {
  try {
    ...
    if (!req.user || req.user.role !== "delivery_personnel") {
      return res.status(403).json({
        error: "Unauthorized - Only delivery personnel can access this route",
      });
    }
...
export const protectSystemAdminRoutes = async (req, res, next) => {
  try {
    ...
    if (!req.user || req.user.role !== "system_admin") {
      return res.status(403).json({
        error: "Unauthorized - Only system admins can access this route",
      });
    }
